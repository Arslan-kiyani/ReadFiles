...
namespace ReadFile_Mini.Controllers
{
    ...
    {
        private readonly HelperExcel _helperExcel;
        public FileReadController(HelperExcel helperExcel)
        {

            _helperExcel = helperExcel;
        ...
        public async Task<IActionResult> Upload([FromForm] ExcelFileUploadModel file)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                stream.Position = 0;
                dataList = await _helperExcel.ReadExcel(stream);
            }
            return Ok(dataList);
        }
    }
